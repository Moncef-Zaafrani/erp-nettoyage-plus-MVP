...
## Step 0: Before you start (prep)
- Inventory current UX & endpoints: list all UI screens, API endpoints, and permissions currently used by the Users module, and any related code. Review them and decide what must be removed or changed. In UI part we keep the theme and the professional feel.
- Backup & snapshot: take a backup of existing user data and export sample CSVs for testing.
- Create a staging branch and a feature flag for the redesigned Users page, this will be a dangerous work.
- Note for backend: plan database rework in backend as needed for new fields and for batch actions. (You’ll implement DB changes later with the AI dev flow. We are using supa base for both production and dev, Railway for backend in production and locally for dev, and Versel for frontend in production and locally for dev).
- Seed data: prepare a set of test users representing every role & edge-case (Super Admin, Admin, Superviseur for multiple zones, Agents assigned to multiple sites, Clients current/former/archived).
- Read the rest of the documents for context, then read it a second time to know wich backend code need to be modified or added or removed. Read it a third time to make sure everything is perfect and your todo list is well documented and detailed, then remove the old code and start working again.
